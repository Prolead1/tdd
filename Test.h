...
namespace MereTDD {
  ...
  class ActualConfirmException: public ConfirmException {
    public:
      ActualConfirmException(std::string_view expected, std::string_view actual, int line) : ConfirmException(line), mExpected(expected), mActual(actual){
        formatReason();
      }
  ...
  inline void confirm(std::string &expected, std::string &actual, const std::source_location location = std::source_location::current()) {
    if (actual != expected) {
      throw ActualConfirmException(std::string_view(expected), std::string_view(actual), location.line());
    }
  }
  ...
  inline std::map<std::string, std::vector<TestSuite*>> &getTestSuites() {
    static std::map<std::string, std::vector<TestSuite*>> testSuites;
    return testSuites;
  }
  ...
  class TestBase {
    public:
      TestBase(std::string_view name, std::string_view suiteName): mName(name), mSuiteName(suiteName), mPassed(true), mConfirmLocation(-1) { };
  ...
  inline void runTest(std::ostream &output, Test *test, int &numPassed, int &numFailed, int &numMissedFailed) {
      ...
      if (test->passed()) {
        ...
        if (test->confirmLocation() != -1) {
          output << "Failed test on line " << test->confirmLocation() << "\n";
    ...
    for (auto &suite: getTestSuites()[name]){
      ...
      if (suite->passed()){
        ...
        if(suite->confirmLocation() != -1){
          output << "Failed confirm on line " << suite->confirmLocation() << "\n";
  ...
  inline bool runTests(std::ostream &output){
    output << "Running " << getTests().size() << " test suites." << std::endl;
    int numPassed = 0;
    int numFailed = 0;
    ...
    for (auto const &[key, value]: getTests()){
      ...
      if (not key.empty()){
        ...
        if (not runSuite(output, true, key, numPassed, numFailed)){
          output << "Test suite setup failed. Skipping tests in suite." << std::endl;
          continue;
...
class MERETDD_CLASS: public MereTDD::Test { \
  public: \
      MERETDD_CLASS(std::string_view name, std::string_view suiteName): Test(name, suiteName) { } \
      void run() override; \
}; \
} \
...
class MERETDD_CLASS: public MereTDD::TestEx<exceptionType> { \
  public: \
      MERETDD_CLASS(std::string_view name, std::string_view suiteName, std::string_view exceptionName): TestEx(name, suiteName, exceptionName) { } \
      void run() override; \
}; \
} \
